...
public class ChangePassword extends BaseActivity {
                ...
                if (password.equals(confirmPassword)) {
                    if (isValidPassword(password)) {
                        if (password.equals(getOldPassword())) {
                            ...
                            makeToast("Password changed successfully", getApplicationContext());
    ...
    public boolean isValidPassword(String password) {
        if (!hasAtLeastTwoNumbers(password)) {
            makeToast("Your password needs two numbers", this);
            return false;
        }

        if (!hasASpecialSymbol(password)) {
            makeToast("Your password needs a special symbol", this);
            return false;
        }

        if (containsSpace(password)) {
            makeToast("Your password cannot contain spaces", this);
            return false;
        }
